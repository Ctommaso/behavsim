...
def tumble_dryer(param, res_dir, text_view):
        ...
        for j in range(int(round(param["Number of Days"][0]))):
            ...
            if cycles != 0:
                ...
                for k in range(cycles):
                    ...
                    l2 = l1+sub.rd_var.norm(param["High Duration [min]"][0], param["High Duration [min]"][1], True)
                    
                    # Computes the parameters of the first phase.
                    power = sub.rd_var.norm(param["High Power [W]"][0], param["High Power [W]"][1], False)
                    active_power[l1:l2] += power
                    l1 = l2
                    
                    # Computes the duration of the third phase.
                    l2 = l1+sub.rd_var.norm(param["Low Duration [min]"][0], param["Low Duration [min]"][1], True)
                    
                    # Computes the parameters of the second phase.
                    power = sub.rd_var.norm(param["Low Power [W]"][0], param["Low Power [W]"][1], False)
                    active_power[l1:l2] += power
                    l1 = l2
